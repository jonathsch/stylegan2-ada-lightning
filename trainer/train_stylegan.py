...
from dataset.stable_diffusion import StableDiffusionShapeNetDataset
...
class StyleGAN2Trainer(pl.LightningModule):
    def __init__(self, config):
        ...
        self.train_set = StableDiffusionShapeNetDataset(
            config.dataset_path,
            config.image_size,
            shapenet_split_file=config.split_file,
        )
        self.val_set = StableDiffusionShapeNetDataset(
            config.dataset_path,
            config.image_size,
            config.num_eval_images,
            shapenet_split_file=config.split_file,
        )
    ...
    def configure_optimizers(self):
        g_opt = torch.optim.Adam(
            list(self.G.parameters()), lr=self.config.lr_g, betas=(0.0, 0.99), eps=1e-8
        )
        d_opt = torch.optim.Adam(
            self.D.parameters(), lr=self.config.lr_d, betas=(0.0, 0.99), eps=1e-8
        )
        return g_opt, d_opt
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (
            self.global_step > self.config.lazy_path_penalty_after
            and (self.global_step + 1) % self.config.lazy_path_penalty_interval == 0
        ):
            g_opt.zero_grad(set_to_none=True)
        ...
        if (self.global_step + 1) % self.config.lazy_gradient_penalty_interval == 0:
            ...
            for acc_step in range(total_acc_steps):
                ...
                gp_loss = (
                    ...
                    * gp
                    * self.config.lazy_gradient_penalty_interval
                )
    ...
    def latent(self, limit_batch_size=False):
        batch_size = (
            ...
            else self.config.batch_gpu // self.path_length_penalty.pl_batch_shrink
        )
